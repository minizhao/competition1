...
def data_hlper(sub_usr_df,sub_shop_df,mall_w2idx):
    ...
    for index, row in sub_usr_df.iterrows():
        ...
        mall_wifi_f=get_wifi_f(wifi_str=row['wifi_infos'],mall_w2idx=mall_w2idx)
        time_point=get_time_point(row['time_stamp'])
        usr_id=us_name_2_idx(row['user_id'])
        x_data.append(np.concatenate([cluster_label,top5_wifi_name,mall_wifi_f,conn_w_n,time_point,[usr_id]]))
        y_data.append(sf_name_2_idx(row['shop_id']))

    assert len(x_data)==len(y_data)
    return x_data,y_data

def val_data_loader(sub_usr_df,sub_shop_df,mall_w2idx):
    x_data=[]
...
def get_wifi_ofmall(mall_df=''):
    all_wifi=[]
    for index, row in sub_usr_df.iterrows():
        wifi_list=[wifi.split('|')[0] for wifi in row['wifi_infos'].split(';')]
        all_wifi.extend(wifi_list)
    w_count=collections.Counter(all_wifi)
    w_select=[x for x in w_count.keys() if w_count[x]>=10]
    mall_w2idx = dict(zip(w_select, range(len(w_select))))
    return  mall_w2idx


def get_wifi_f(wifi_str='',mall_w2idx=''):
    list_f=np.zeros(len(mall_w2idx))
    wifi_list=[wifi.split('|') for wifi in wifi_str.split(';')]
    for item in wifi_list:
        if item[0] in mall_w2idx.keys():
            list_f[mall_w2idx[item[0]]]=item[1]
    return list_f
...
if __name__ == '__main__':
    ...
    for idx,ma_id in all_mallid:
        ...
        mall_w2idx=get_wifi_ofmall(sub_usr_df)

        x_data,y_data=data_hlper(sub_usr_df,sub_shop_df,mall_w2idx)
        x_test_data=val_data_loader(sub_test_df,sub_shop_df,mall_w2idx)
